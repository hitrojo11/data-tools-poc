...
def main():
    ...
    if uploaded_file is not None:
        ...
        if size_mb is not None and size_mb > max_file_mb:
            st.error(
                f"Uploaded file is {size_mb:.1f} MB which exceeds the limit of {max_file_mb} MB. Please increase the limit or upload a smaller file."
            )
        else:
    ...
    for col in work_df.columns:
        if work_df[col].dtype == "object":
            ...
            if (
                len(types_seen - set(["str", "float", "int", "bool", "NoneType"])) > 0
                or len(types_seen) > 1
            ):
    ...
    if mixed_report:
        with st.expander("Columns with mixed types (sampled)", expanded=False):
            ...
            st.write(
                "You can coerce these columns using the Schema UI below or the coercion tool in the column actions."
            )
    ...
    col_action = st.selectbox(
        "Select column to act on", options=["--"] + list(work_df.columns)
    )
    if col_action != "--":
        ...
        with c1:
            ...
            target_type = st.selectbox(
                "To type",
                options=["numeric", "int", "float", "datetime", "string", "category"],
                key="coerce_type",
            )
            if st.button("Coerce column"):
                ...
                try:
                    ...
                    details = {
                        ...
                        "action": "coerce",
                        "to": target_type,
                        "before_non_null": int(before_non_null),
                        "after_non_null": int(after_non_null),
                    }
                    log_action("coerce", details)
                    st.success(
                        f"Attempted coercion to {target_type}. Non-null before: {before_non_null}, after: {after_non_null}"
                    )
                except Exception as e:
            ...
            if st.button("Apply impute"):
                if work_df[col_action].isna().sum() == 0:
                    ...
                    if pd.api.types.is_numeric_dtype(work_df[col_action]):
                        before_stats = {
                            "mean": float(work_df[col_action].mean()),
                            "median": float(work_df[col_action].median()),
                        }
                    try:
                        ...
                        details = {
                            ...
                            "before_missing": before_count,
                            "after_missing": after_count,
                            "before_stats": before_stats,
                            "after_stats": after_stats,
                        }
                        log_action("impute", details)
                        st.success(
                            f"Imputation applied. Missing before: {before_count}, after: {after_count}"
                        )
    ...
    if num_cols:
        sel = st.selectbox(
            "Select numeric column for outlier scan",
            options=num_cols,
            key="outlier_col",
        )
    ...
    if openai_client is not None:
        ...
        st.info(
            "OpenAI client not initialized. Set OPENAI_API_KEY in sidebar to enable LLM insights."
        )
